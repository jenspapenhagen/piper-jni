...
public class PiperConfig extends PiperJNI.JNIRef {
     ...
     */
    protected PiperConfig(final PiperJNI piper) throws IllegalArgumentException {
        super(piper.newConfig());
        this.piper = piper;
    }

    protected void setESpeakDataPath(final Path path) throws FileNotFoundException {
    ...
    protected void setTashkeelModelPath(final Path path) throws FileNotFoundException {
        if (path != null && (!Files.exists(path) || Files.isDirectory(path))) {
